...
(function( root, factory ) {
	...
	function setupDOM() {
		...
		createSingletonNode( dom.wrapper, 'aside', 'controls',
			'<div class="container"><div class="button-container"><button class="navigate-left" aria-label="previous slide"></button>' +
			'<button class="navigate-right" aria-label="next slide"></button>' +
			'<button class="navigate-up" aria-label="above slide"></button>' +
			'<button class="navigate-down" aria-label="below slide"></button></div></div>' );
	...
	function layout() {
		...
		if( dom.wrapper && !isPrintingPDF() ) {
			...
			if( scale <= 0.6 ) {
				...
				transformSlides( { layout: 'translate(-50%, -50%) scale(0.7)' } );
			} else if( scale >= 0.7 || scale === 1 ) {// Don't apply any scaling styles if scale is 1
				dom.slides.style.zoom = '';
